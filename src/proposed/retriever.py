...
from src.proposed.vector_db import VectorDB
...
class HybridRetriever:
    ...
    def _hybrid_merge(
        ...
        filtered_hits = [
            hit for hit in sorted_hits if hit["hybrid_score"] >= self.RRF_THRESHOLD
        ]

        if filtered_hits:
            logger.info(f"Top hybrid score: {filtered_hits[0]['hybrid_score']:.5f}")
        else:
            logger.info(f"No candidates passed RRF threshold ({self.RRF_THRESHOLD})")
    ...
    def retrieve(
        self, query: str, final_top_k: int = FINAL_TOP_K, use_rerank: bool = True
    ) -> List[Dict[str, Any]]:
...
def build_retriever() -> HybridRetriever:
    ...
    if existing_count == 0:
        logger.info(
            "-> Database Chroma rỗng! Đang tiến hành tạo dữ liệu VectorDB (Chỉ chạy 1 lần)..."
        )
        db.build_db(batch_size=32)
    else:
        logger.info(
            f"-> Dữ liệu VectorDB đã tồn tại ({existing_count} chunks). Bỏ qua tạo mới."
        )
