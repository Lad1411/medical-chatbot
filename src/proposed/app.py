...
async def lifespan(app: FastAPI):
    ...
    if not DISABLE_RAG:
        ...
        try:
            import src.proposed.vector_db as _vdb_mod
...
async def root():
    ...
    return HTMLResponse(
        content="<h1>index.html not found in ./static/</h1>", status_code=404
    )
...
async def chat(req: ChatRequest):
    ...
    return ChatResponse(
        answer=answer, retrieved_docs=n_docs, latency_ms=round(latency_ms, 1)
    )
...
if __name__ == "__main__":
    ...
    uvicorn.run("src.proposed.app:app", host="0.0.0.0", port=port, reload=False)
