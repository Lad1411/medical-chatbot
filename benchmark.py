"""
benchmark.py — Full Medical Benchmark Suite
============================================
Runs all evaluation phases in sequence (one script, no manual steps):

  Phase 1 │ PubMedQA  │ RAG-SFT LoRA  │ Built-in abstract context
  Phase 2 │ MedQA     │ Base model    │ No RAG  (zero-shot baseline)
  Phase 3 │ MedQA     │ CoT-200 LoRA  │ No RAG
...
Usage:
  python benchmark.py \
      --ragsft-lora /path/to/ragsft_lora \
      --cot200-lora /path/to/cot200_lora \
      [--limit 100] \
      [--output results.csv] \
      [--phases all]          # or e.g. "1,3,5" to run specific phases
"""
...
PHASES = [
    {
        "id":             1,
        "name":           "PubMedQA | RAG-SFT + Built-in Context",
        "model_key":      "ragsft",
        "dataset":        "pubmedqa",
        "retriever_mode": "builtin",
    },
    {
        "id":             2,
        "name":           "MedQA | Base + No RAG",
        "model_key":      "base",
        "dataset":        "medqa",
        "retriever_mode": "none",
    },
...
# ==========================================
def parse_args():
    parser = argparse.ArgumentParser(description="Full Medical Benchmark Suite")
    parser.add_argument(
        "--ragsft-lora", type=str, default="",
        help="Path to RAG-SFT LoRA adapter (required for Phase 1)"
    )
    parser.add_argument(
        "--cot200-lora", type=str, default="",
        help="Path to CoT-200 LoRA adapter (required for Phases 3-6)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Comma-separated phase IDs to run, e.g. '1,2,3' or 'all'"
    )
...
def main():
    ...
    lora_map = {
        "base":   None,
        "ragsft": args.ragsft_lora if args.ragsft_lora else None,
        "cot200": args.cot200_lora if args.cot200_lora else None,
    }
    for phase in phases_to_run:
        key = phase["model_key"]
        if key != "base" and not lora_map[key]:
            print(f"[!] Phase {phase['id']} requires --{key}-lora but it was not provided.")
            return

    print("=" * 65)
    print("  MEDICAL BENCHMARK SUITE")
    print("=" * 65)
    print(f"  Phases to run : {[p['id'] for p in phases_to_run]}")
    print(f"  RAG-SFT LoRA  : {lora_map['ragsft'] or 'N/A'}")
    print(f"  CoT-200 LoRA  : {lora_map['cot200'] or 'N/A'}")
    print(f"  Limit         : {args.limit if args.limit > 0 else 'full dataset'}")
    print(f"  Output        : {args.output}")
    print("=" * 65)

    # ── Build retriever only if a retriever-based phase is selected ──
    needs_retriever = any(
        p["retriever_mode"] not in ("none", "builtin") for p in phases_to_run
    )
    retriever = None
    if needs_retriever:
    ...
    # ── Load datasets ────────────────────────────────────────────────
    needs_medqa   = any(p["dataset"] == "medqa"    for p in phases_to_run)
    needs_pubmed  = any(p["dataset"] == "pubmedqa" for p in phases_to_run)
    ...
    if needs_pubmed:
        # pqa_labeled has 1 000 expert-labelled examples, no 'test' split — use 'train'
    ...
    for phase in phases_to_run:
        ...
        if phase["dataset"] == "pubmedqa":
            correct, total = run_pubmedqa_phase(
                phase, model, tokenizer, pubmedqa_ds, all_results
            )
        else:
