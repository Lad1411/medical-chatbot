...
def parse_args():
    ...
    parser.add_argument(
        "--lora-path",
        type=str,
        default="./models/checkpoint-1600",
    )
    ...
    for start_idx in range(0, total, BATCH_SIZE):
        ...
        for item in batch:
            ...
            if use_rag and retriever_engine is not None:
                ...
                hits = retriever_engine.retrieve(
                    question,
                    final_top_k=3,
                    use_rerank=True
                )
...
def main():
    ...
    if args.limit > 0:
        ...
        medqa_ds = medqa_ds.select(
            range(min(args.limit, len(medqa_ds)))
        )

        pubmedqa_ds = pubmedqa_ds.select(
            range(min(args.limit, len(pubmedqa_ds)))
        )

    results_data = []

    med_correct, med_total = run_medqa_eval(
        medqa_ds,
        model,
        tokenizer,
        retriever_engine,
    ...
    pub_correct, pub_total = run_pubmedqa_eval(
        pubmedqa_ds,
        model,
        tokenizer,
        retriever_engine,
    ...
    med_acc = (
        med_correct / med_total * 100
    )

    pub_acc = (
        pub_correct / pub_total * 100
    )
